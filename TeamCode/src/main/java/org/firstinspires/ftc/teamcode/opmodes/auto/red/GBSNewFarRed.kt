...
import org.firstinspires.ftc.teamcode.config.GBSFarRedConfiguration
...
@Suppress("unused")
@Autonomous(name = "GBSNewFarRed", group = "Red")
class GBSNewFarRed : GBSOpMode() {
    ...
    override fun run(): Result<Unit> {
        ...
        base.autoDrive(
            GBSFarRedConfiguration.BASE_POWER,
            GBSFarRedConfiguration.MOTOR_DISTANCES.first,
            GBSFarRedConfiguration.MOTOR_DISTANCES.second,
            5000,
            {
                base.autoDrive(GBSFarRedConfiguration.BASE_POWER, 12, -12, 5000, {
                    sleep(GBSFarRedConfiguration.SPINUP_MS)
        ...
        if (webcam2.aprilTagDetections.isNotEmpty()) {
            ...
            val errorYaw =
                GBSFarRedConfiguration.DESIRED_TAG_ORIENTATION.yaw - currentPose.orientation.yaw

            val turnPower = errorYaw * GBSFarRedConfiguration.PID_K_P

            if (abs(errorYaw) > GBSFarRedConfiguration.ERROR_EPSILON) {
                base.autoPower(
                    GBSFarRedConfiguration.ERROR_CORRECTION_SPEED, -turnPower, turnPower
                )
            } else {
